...
case class GroupRing2(group: FiniteGroup, ring:Ring ) extends Ring {
  ...
  val zero = GroupRing2Element(List.fill(dim)(rngZero).toVector)
  // El 0 del GroupRing
  val one = GroupRing2Element(List.fill(dim - 1)(rngZero).toVector.+:(rngOne))
  // El 1 del GroupRing
  ...
  override val structureId: String = "GroupRing" + group.structureId + ring.structureId

  def builder(x:T1): T2 = GroupRing2Element(x)
  ...
  case class GroupRing2Element(vector: Vector[ring.T2]) extends RingElement {
    ...
    def add(other: GroupRing2Element) = {
      ...
      GroupRing2Element(recursiveAdd(this.vector, other.vector))
    }
    ...
    def multiply(other: GroupRing2Element) = {
      ...
      GroupRing2Element(tmp7)
    }

    def multiply(other: ring.T2): GroupRing2Element = {
      val tmp = GroupRing2Element(List.fill(dim - 1)(rngZero).toVector.+:(rngOne.multiply(other)))
      this.multiply(tmp)
    }
